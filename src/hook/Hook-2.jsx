...
function App() {
  ...
  // 의존성 배열 넣지 않을 경우, 리랜더링될때마다 useEffect가 실행되었다면,
  // 배열을 넣으면 배열의 값이 바뀔때와 처음 랜더링 됐을때 실행된다(첫 랜더링 시 실행은 무조건임)

  // 1. 랜더링하면 useEffect가 먼저 실행된다
  // 2. input 에 값을 입력하면 value, 즉 state(상태)가 변경됨
  // 3. state가 변경되었기 때문에 App 컴포넌트가 리랜더링된다
  // 4. 리랜더링이 되면 useEffect가 다시 실행된다 input 값이 바뀔때마다 리랜더링 되고, useEffect가 계속 실행됨
  // 5. useEffect가 실행될때마다 리랜더링이 되고 있는 것
...
export default App;
// useEffect
// - 랜더링 될 때, 특정한 작업을 수행해야 할 때 설정하는 훅 (ex.alert, console... 등)
